...
**完成度: 55%** | **商品登録: 95%** ✅ | **在庫管理: 0%** | **設定管理: 100%** ✅ | **売上分析: 20%** | **管理番号システム: 100%** ✅ | **ハッシュタグシステム: 100%** ✅ | **セールスワード設定: 100%** ✅ | **使い方ガイド: 100%** ✅ | **リセット機能: 100%** ✅ | **コピー機能: 100%** ✅ | **商品名並び替え: 100%** ✅ | **コード品質: 85%** 🔧
...
**最終更新日**: 2025年10月12日（商品名ブロック並び替え機能 + 仕入・出品日設定強化完成）

**最新の更新内容**:
- 📊 完成度を更新: 全体50%、商品登録90% ✅
- ✅ 商品名ブロックのドラッグ&ドロップ並び替え機能完成 ★目玉機能
  - ⋮⋮ ドラッグハンドルで各ブロックを自由に並び替え
  - セールスワード、ブランド情報、アイテム名、商品属性のすべてを自由配置
  - 設定マスタに並び順を保存、次回起動時も同じ順序で表示
  - リアルタイムプレビュー更新（並び替えに応じて即座に反映）
...
- ✅ アイテム名の独立ブロック化完成
  ...
  - 他のブロックと同様にドラッグ&ドロップで並び替え可能
- ✅ 仕入・出品デフォルト設定の強化
  - 「常に今日の日付を使用」チェックボックス追加
  - 固定日付入力フィールド追加（カレンダー）
  - チェック時は日付フィールドを無効化（グレーアウト）
  - 仕入日・出品日を柔軟に設定可能
- 🔄 設定管理タブの並び替え
  - 管理番号設定 → セールスワード → 商品状態ボタン → 割引情報 → ハッシュタグ → 配送デフォルト → 仕入・出品デフォルト → 使い方ガイド
- 📋 実装ファイル:
  - `sp_block_title.html`: ドラッグ可能ブロック構造、アイテム名ブロック追加
  - `sp_scripts.html`: ドラッグ&ドロップロジック、並び順保存/読み込み、後方互換性
  - `config_loader.js`: 商品名ブロック並び順の保存/取得関数追加
  - `sidebar_config.html`: 仕入・出品日設定UI、トグル関数
