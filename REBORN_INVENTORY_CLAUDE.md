...
**Claude Code（AI開発アシスタント）へ**: 会話開始時（新規/継続問わず）に必ず実行すること

### 📋 必須チェックリスト（毎回実行）

**1. Issue管理ドキュメントの確認:**
```markdown
- [ ] docs/issues.md を読み込む（未完了Issueの確認）
- [ ] docs/TDD_POLICY.md を確認（Issue管理・TDDルールの理解）
```

**2. 開発前の確認:**
```markdown
- [ ] 新しい作業を始める前に、関連Issueがないか確認
- [ ] 不具合/要望が発生したら、必ずIssue起票
- [ ] 修正完了後は、必ずissues-closed.mdに移動
```

**⚠️ このチェックを怠ると、重要なルール（TDD、Issue管理）を見逃します。**
