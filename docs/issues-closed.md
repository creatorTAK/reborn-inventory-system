## PROD-002 | 機能追加: 商品登録時の発送方法・梱包資材の事前選択 ✅ DONE (完了日: 2025-12-22)

### 📌 基本情報
- [x] カテゴリ: 機能追加
- [x] 優先度: 高
- [x] 影響範囲: 商品登録画面、販売記録機能
- [x] 要望日: 2025-11-21
- [x] 完了日: 2025-12-22
- [x] 完了理由: QRコード導入により運用ルール変更。一旦白紙に戻し、必要時に再検討

### 💡 概要
商品登録時に発送方法・梱包資材を事前選択し、販売時は価格入力のみで完了する機能。
...
## HELP-001 | 機能追加: AIサポートチャットボット（ヘルプページ） ✅ DONE (完了日: 2025-12-22)

### 📌 基本情報
- [x] カテゴリ: 機能追加
- [x] 優先度: 中
- [x] 影響範囲: ヘルプページ（help.html）、Cloudflare Workers
- [x] 要望日: 2025-12-15
- [x] 完了日: 2025-12-22
...
- ヘルプページにAIチャットボットUI実装
- Gemini API連携
- 基本的なQ&A機能

---

## UI-015 | UI改善: チャットメニュー追加（トップメニュー・サイドメニュー） ✅ DONE (完了日: 2025-11-19)

### 📌 基本情報
- [x] カテゴリ: UI改善
- [x] 優先度: 低
- [x] 影響範囲: PWA - トップメニュー、サイドメニュー
- [x] 発見日: 2025-11-15
- [x] 完了日: 2025-11-19

### 💡 要望内容

トップメニュー（menu_home.html）とサイドメニュー（docs/index.html）にチャット機能へのリンクを追加し、アクセス性を向上させる。

**背景:**
- 現在、チャット機能へはヘッダーアイコンからのみアクセス可能
- メニューからもアクセスできると利便性が向上

### ✅ 期待動作

**トップメニュー（menu_home.html）:**
```
メインメニュー
├─ 商品登録
├─ 在庫管理
...
**GAS:**
- `menu_home.html` - トップメニューにチャット項目追加

**PWA:**
- `docs/index.html` - サイドメニューにチャット項目追加

### ✏️ 実装内容

- [x] menu_home.html: チャット項目を「入出庫履歴」と「マスタ管理」の間に追加
- [x] navigateToPage('chat') 関数呼び出し実装
- [x] docs/index.html: サイドメニューにチャット項目追加
- [x] drawer-chat アイテムにアクティブ状態反映

### 状態
- [x] ✅ DONE (完了日: 2025-11-19)

---

## MASTER-001 | 機能追加: ブランドマスタ管理画面実装（追加・削除機能） ✅ DONE (完了日: 2025-11-14)
...
- 開始日: 2025-11-14
- 完了日: 2025-11-14
- 関連Issue: ARCH-001-3.2（ブランドマスタFirestore移行完了）
- デプロイ: PWA 0c35e74, GAS @860

### 💡 目的
ブランドマスタをスマホから直接管理できるUI実装。
スプレッドシート編集に依存せず、PWA上でブランドの追加・削除が可能に。
SaaS化基盤構築の第一歩。

### ✅ 完了した実装

#### Phase 1: 基本機能（MVP）
- ✅ ブランド一覧画面（master-brand-list.html）
  - Firestoreから全ブランド取得（51,342件）
  - スクロール可能なリスト表示
  - 検索・フィルター機能（ひらがな対応）
  - 使用頻度表示
  - パフォーマンス最適化（表示上限100件、デバウンス500ms）
...
### 📍 作成ファイル
- `docs/master-brand-list.html` - ブランド一覧画面
- `docs/js/master-brand-manager.js` - ブランド管理ロジック

### 📍 修正ファイル
- `docs/js/firestore-api.js` - ブランドCRUD API追加
  - `async function createBrand(nameEn, nameKana)`
  - `async function deleteBrand(brandId)`
  - キャッシュ更新機能実装

### 🧪 テスト結果
- ✅ TC-MASTER-001-001: ブランド追加 → PASS
- ✅ TC-MASTER-001-002: ブランド削除 → PASS
- ✅ TC-MASTER-001-003: 一括削除 → PASS
- ✅ TC-MASTER-001-004: ひらがな検索 → PASS
- ✅ TC-MASTER-001-005: 実機テスト → PASS（ユーザー確認済み）
