...
## CHAT-002 | バグ修正: 初回チャットルーム未開封端末で通知・バッジ未達
...
### ✅ 期待動作
- 初回アクセス時から、チャットルームを開いていなくても通知・バッジが届くべき
- FCMトピック `all_users` にサブスクライブされていれば通知が届くはず

### 📍 関連ファイル
- `docs/index.html` (FCM初期化、トピックサブスクリプション)
- `docs/firebase-messaging-sw.js` (サービスワーカー、バックグラウンド通知)
