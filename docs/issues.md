...
#### Phase 1: 基盤整備
- [ ] SKUコレクション設計・作成
- [ ] purchaseSlots拡張（itemType, quantity, skuId）
- [ ] products拡張（itemType, skuId, variations）
- [ ] Firestoreルール更新

#### Phase 2: 仕入登録対応
- [ ] 商品タイプ選択UI（1点物/新品）
- [ ] 数量入力（同一商品×N）
- [ ] バリエーション設定UI（サイズ×カラーマトリクス）
- [ ] SKU自動生成ロジック
- [ ] QR方式選択（個別/SKU）
...
## PROD-002 | 機能追加: 商品登録時の発送方法・梱包資材の事前選択

### 📌 基本情報
- [ ] カテゴリ: 機能追加
- [ ] 優先度: 高
- [ ] 影響範囲: 商品登録画面（product.html）、販売記録機能
- [ ] 要望日: 2025-11-21
- [ ] 関連Issue: TEST-001（商品登録画面テスト中）

### 💡 背景・目的
**背景:**
- 現在の販売フロー：商品が売れた後に発送方法・梱包資材を手動選択
- 自動化の障壁：メルカリ（通常版）はAPI未対応、通知のみ
- SaaS戦略：Freeプラン（副業ユーザー）= 半自動化、Proプラン（本格事業）= 完全自動化

**問題点:**
- 販売後の入力は手間がかかる
- 入力ミスのリスクが高い
- 商品IDをコピー→検索→入力という煩雑な手順

**解決策:**
商品登録時に発送方法・梱包資材を事前選択しておくことで、販売後の入力作業を最小化する。

**自動化レベルによる差別化:**
| プラン | 対象プラットフォーム | 自動化レベル | 商品登録時の設定 |
|--------|---------------------|-------------|----------------|
| Free | 通常メルカリ | 半自動（Gmail転送） | 発送方法・梱包資材選択 → 販売時は価格入力のみ |
| Standard | メルカリShops/BASE | 半自動（バーコード） | 発送方法・梱包資材 + バーコード生成 |
| Pro | BASE/Shopify | 完全自動（API統合） | API自動連携、手動入力不要 |

### ✅ 期待動作

#### 1. 商品登録画面の改善
**追加するUI要素:**
- 発送方法プルダウン（梱包資材マスタから取得）
- 梱包資材プルダウン（梱包資材マスタから取得）
- 配置位置：「基本情報」セクション内、他のフィールドと同様

**保存される情報:**
- 商品データに `shippingMethod` と `packagingMaterial` を追加
- Firestoreの `products` コレクションに保存

#### 2. 販売記録機能との連携
**販売時の動作:**
1. 商品ID検索時に事前選択した発送方法・梱包資材を自動表示
2. ユーザーは販売価格のみ入力（発送方法・梱包資材は変更可能）
3. 利益計算は従来通り自動実行

**Freeプラン想定フロー（通常メルカリ）:**
1. メルカリで商品が売れる → 通知メール受信
2. 商品ID（11桁）をコピー
3. 在庫管理画面でID検索
4. 発送方法・梱包資材が自動表示（事前選択済み）
5. 販売価格のみ入力 → 保存

### 📍 関連ファイル
- `docs/product.html` - 商品登録画面UI
- `docs/js/product-scripts.js` - 商品登録ロジック
- Firestore: `products` コレクション（スキーマ拡張）
- 梱包資材マスタ（INV-004で実装済み）

### 🧪 テストケース

#### TC-PROD-002-001: 商品登録時の発送方法・梱包資材選択
**前提条件:**
- 梱包資材マスタに「ゆうパケット」「宅急便コンパクト」等が登録済み
- ユーザーがログイン済み

**実行操作:**
1. 商品登録画面を開く
2. 基本情報入力（商品名、管理番号等）
3. 発送方法プルダウンから「ゆうパケット」を選択
4. 梱包資材プルダウンから「クリックポスト用封筒」を選択
5. 「登録」ボタンをクリック

**期待結果:**
- Firestoreの `products` コレクションに以下が保存されること:
  - `shippingMethod: "ゆうパケット"`
  - `packagingMaterial: "クリックポスト用封筒"`
- 商品登録完了メッセージが表示されること

#### TC-PROD-002-002: 販売記録時の自動表示
**前提条件:**
- TC-PROD-002-001で登録した商品が在庫として存在
- 商品が販売済みステータス

**実行操作:**
1. 在庫管理画面で商品ID検索（メルカリ11桁ID）
2. 販売記録モーダルを開く

**期待結果:**
- 発送方法フィールドに「ゆうパケット」が自動表示
- 梱包資材フィールドに「クリックポスト用封筒」が自動表示
- ユーザーは販売価格のみ入力すればよい

#### TC-PROD-002-003: デグレード確認
**前提条件:**
- 既存の商品データ（shippingMethod/packagingMaterial未設定）
...
**期待結果:**
- エラーが発生しないこと
- 発送方法・梱包資材フィールドは空欄で表示されること
- 従来通り手動入力できること

### ✏️ 実装内容
- [ ] Firestoreスキーマ拡張（products コレクション）
- [ ] product.htmlにUIフィールド追加（発送方法・梱包資材プルダウン）
- [ ] product-scripts.jsに保存ロジック追加
- [ ] 梱包資材マスタからのデータ取得実装
- [ ] 販売記録モーダルに自動表示ロジック追加
- [ ] TC-PROD-002-001実行（新規登録テスト）
- [ ] TC-PROD-002-002実行（販売記録自動表示テスト）
- [ ] TC-PROD-002-003実行（デグレード確認）

### 📝 テスト結果
- [ ] TC-PROD-002-001: PASS / FAIL
- [ ] TC-PROD-002-002: PASS / FAIL
- [ ] TC-PROD-002-003: PASS / FAIL
- [ ] デグレード確認: OK / NG

### 🔗 関連情報
**SaaS戦略との関連:**
- Freeプラン: この機能により手動入力の手間を大幅削減
- Standardプラン: バーコード機能と組み合わせてさらに効率化
- Proプラン: API統合により完全自動化

**将来の拡張:**
- 商品ID検索機能（PROD-003として別途起票予定）
- Gmail転送パース自動化（Phase 2）
- BASE/Shopify API統合（Phase 3）
...
## HELP-001 | 機能追加: AIサポートチャットボット（ヘルプページ）

### 📌 基本情報
- [ ] カテゴリ: 機能追加
- [ ] 優先度: 中
- [ ] 影響範囲: ヘルプページ（help.html）、Cloudflare Workers
- [ ] 要望日: 2025-12-15
...
**背景:**
- 現在のヘルプページはFAQとガイドのみ
- ユーザーは困った時に「誰かに質問できる」のが最も助かる
- SaaS化に向けて、サポートコスト削減と24時間対応が必要

**目的:**
- システムに関するQ&Aにいつでも回答できるAIチャットボット
- よくある質問やガイドにない問題にも対応
- ユーザー満足度向上とサポート負荷軽減
...
**ユーザー体験:**
1. ヘルプページにチャットUIが表示される
2. 質問を入力すると、AIが即座に回答
3. システム固有の情報（FAQ、ガイド、マニュアル）に基づいた回答
4. 会話履歴が保持され、文脈を理解した回答が可能

**技術要件:**
- Gemini API使用（既存の商品説明生成と同じ）
- RAG方式：関連ドキュメントをコンテキストとして送信
...
**PWA:**
- `docs/help.html` - ヘルプページ（チャットUI追加）
- `docs/js/help-chatbot.js` - チャットボットロジック（新規）
- `docs/css/help-chatbot.css` - チャットUI用スタイル（新規）

**Cloudflare Workers:**
- `workers/help-chatbot/` - Gemini API呼び出しエンドポイント（新規）

**Firestore:**
- `helpChatHistory` コレクション - 会話履歴保存（新規）
- `helpKnowledgeBase` コレクション - FAQ・ガイド・マニュアル（新規）

### 🏗️ 実装フェーズ

#### Phase 1: 基本チャット機能（MVP）
- [ ] 1-1: ヘルプページにチャットUI追加（LINE風デザイン）
- [ ] 1-2: Cloudflare WorkerでGemini API呼び出しエンドポイント作成
- [ ] 1-3: 基本的なシステムプロンプト設定（FAQ・ガイド内容を含む）
- [ ] 1-4: 動作テスト・デプロイ

#### Phase 2: ナレッジベース強化
- [ ] 2-1: FAQ・ガイドをFirestoreに構造化保存
- [ ] 2-2: 質問に関連するドキュメントを検索してコンテキストに追加（RAG）
- [ ] 2-3: 回答精度向上のためのプロンプトチューニング

#### Phase 3: 会話履歴・コンテキスト維持
- [ ] 3-1: 会話履歴をFirestoreに保存
- [ ] 3-2: 過去の会話を参照した文脈理解
- [ ] 3-3: ユーザーごとのチャット履歴表示

#### Phase 4: SaaS対応・分析機能
- [ ] 4-1: プラン別利用制限（Free: 月10回、Pro: 無制限）
- [ ] 4-2: よくある質問の分析ダッシュボード（管理者向け）
- [ ] 4-3: 未回答・不満足回答の検出とFAQ改善
...
1回の問い合わせ ≒ 2,000トークン
→ 約 $0.0006 / 回（0.1円未満）
→ 月1,000回の問い合わせ ≒ $0.60程度
```

### 🧪 テストケース

#### TC-HELP-001-001: 基本的な質問への回答
**前提条件:**
- ヘルプページにアクセス
- チャットUIが表示されている

**実行操作:**
1. 「商品登録の方法を教えてください」と入力
2. 送信ボタンをクリック

**期待結果:**
- AIが商品登録の手順を回答
- 回答は日本語で、システムのガイドに基づいた内容

#### TC-HELP-001-002: 会話の文脈理解
**実行操作:**
1. 「管理番号とは何ですか？」と質問
2. 回答を確認
3. 「それの設定方法は？」と続けて質問
...
**期待結果:**
- 2回目の質問で「管理番号の設定方法」と理解して回答

### 📝 備考

- 既存のGemini API（商品説明生成）のパターンを参考に実装
- 初期ナレッジベースは既存のFAQ・ガイドから作成
- 段階的にナレッジベースを充実させる
